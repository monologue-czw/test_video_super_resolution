...
try:
    ...
    from models_video.condition2D import UNet2DConditionModel
    from models_video.condition3D import UNet3DConditionModel
...
class UNetVideoModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.conv_change = nn.Conv3d(in_channels=4, out_channels=7, kernel_size=(3, 1, 1), padding=(1, 0, 0))
    ...
    def forward(
        ...
        class_labels: Optional[torch.Tensor] = torch.tensor(20),           #20,
        ...
        sample = self.conv_change(sample)
        #print(sample.shape)
        ...
        for i,(downsample_block, down_temp_block) in enumerate(zip(self.down_blocks, self.down_temp_blocks)):
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                sample, res_samples = downsample_block(
                    ...
                    temb=emb,
                    encoder_hidden_states=encoder_hidden_states[0][8-i],
                    attention_mask=attention_mask,
                )
            else:
                sample, res_samples = downsample_block(hidden_states=sample, temb=emb)
            #print(f"Sample shape after down_block {i}: {sample.shape}")
            ...
            sample = down_temp_block(
                hidden_states=sample,
                encoder_hidden_states=encoder_hidden_states[1][8-i],
                timesteps=timesteps,
                temb=emb,
            )
            print(f"Sample shape after down_temp_block {i}: {sample.shape}")
        ...
        sample = self.mid_block(
            sample,
            emb,
            encoder_hidden_states=encoder_hidden_states[0][4],
            attention_mask=attention_mask
        )
        # 2. temporal modeling at mid block
        sample = self.mid_temp_block(
            hidden_states=sample,
            encoder_hidden_states=encoder_hidden_states[1][4],
            timesteps=timesteps,
            temb=emb,
        )
        print(f"Sample shape after mid_temp_block {i}: {sample.shape}")
        # up
        for i, (upsample_block, up_temp_block) in enumerate(zip(self.up_blocks, self.up_temp_blocks)):
            ...
            if hasattr(upsample_block, "has_cross_attention") and upsample_block.has_cross_attention:
                sample = upsample_block(
                    ...
                    encoder_hidden_states=encoder_hidden_states[0][3-i],
            ...
            sample = up_temp_block(
                hidden_states=sample,
                encoder_hidden_states=encoder_hidden_states[1][3-i],
                timesteps=timesteps,
                temb=emb,
            )
            print(f"Sample shape after up_temp_block {i}: {sample.shape}")
...
'''
# 加载配置文件
with open("C:\\Users\\34936\\Desktop\\Upscale-A-Video-master\\pretrained_models\\upscale_a_video\\unet\\unet_video_config.json", "r") as f:
    unet_config = json.load(f)

# 从配置创建模型
pipeline.unet = UNetVideoModel.from_config(unet_config)

# 加载预训练权重
pretrained_model = "C:\\Users\\34936\Desktop\\Upscale-A-Video-master\pretrained_models\\upscale_a_video\\unet\\unet_video.bin"
...
#encoder_hidden_states = [torch.randn(1, 77, 1024) for _ in range(8)]

encoder_hidden_states=torch.randn(1,77,1024)#这里的B必须是1
